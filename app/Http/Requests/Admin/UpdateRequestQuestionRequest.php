...
class UpdateRequestQuestionRequest extends FormRequest
{
    ...
    {
        $requestQuestion = $this->route('requestQuestion');
        $requestQuestionId = is_object($requestQuestion) ? $requestQuestion->getKey() : $requestQuestion;

        return [
            'code' => ['nullable', 'string', 'max:255', Rule::unique('request_questions', 'code')->ignore($requestQuestionId)],
